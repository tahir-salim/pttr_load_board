                    ...
                    <div class="col-md-6">
                        <div class="tabsShipment">
                            ...
                            <a data-targetit="open" title=""
                                href="{{route(auth()->user()->type.'.my_shipments_overview',[$id])}}">Overview</a>
                            <a class="active" data-targetit="bidactivity" href="javascript:void(0)" title="">
                                Bid Activity</a>
                            @if($shipment->is_tracking == 1)
                            <a data-targetit="tracking" href="{{route(auth()->user()->type.'.my_shipments_tracking',[$id])}}"
                                title="">Tracking</a>
                            @endif
                        </div>
                        <div class="loadboardText">
                            <ul class="d-flex align-items-center">
                                ...
                                <li>Load Board <strong>${{$shipment->dat_rate}}</strong></li>
                                @endif
                                @if($shipment->is_private_network == 1)
                                <li>Private Network <strong>${{$shipment->private_rate}}</strong></li>
                                @endif
                                @if($shipment->max_bid_rate != null)
                                <li>Max Rate <strong>${{$shipment->max_bid_rate}}</strong></li>
                                @endif
                            </ul>
                            {{-- <h4>$ LANE RATE: $344-</h4> --}}
                        </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="javascript:void(0)"
                                                ...
                                                class="rej btn btn-danger">Reject</a>
                                            <a class="accept btn btn-primary" href="javascript:;"
                                                onclick="status_accept_decline('bids',{{$bid->id}},'1')"
                                                title="">ACCEPT</a>
                                            @else
                                            @if($bid->status !== null)
                                            @if ($bid->status == 0)
                                            <a class="rej btn btn-danger" href="javascript:void(0)">REJECTED</a>
                                            @else
                                            <a class="accept btn btn-primary" href="javascript:;" title="">ACCEPTED</a>
                                            @endif
                                            @endif
                                            @endif
                                        </td>
