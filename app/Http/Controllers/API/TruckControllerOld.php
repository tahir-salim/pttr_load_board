...
class TruckController extends Controller
{
    ...
    public function truckPostCreate(Request $request)
    {
        ...
        $truck_post = TruckPost::create([
            ...
            'status_phone' => $request->status_phone == "1" ? Auth::user()->phone : null,
            'status_email' => $request->status_email == "1" ? Auth::user()->email : null,
            'is_posted' => 1,
        ]);
    ...
    public function truckPostList(Request $request)
    {
        ...
        if ($truckPostList instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            $truckPostList->getCollection()->transform(function ($truck) {
                return $this->formatTruckData($truck);
            });
        } else if ($truckPostList) {
            $truckPostList = $this->formatTruckData($truckPostList);
        }
    ...
    public function truckPostUpdate(Request $request, $truck_post_id)
    {
        ...
        $truck_post->truck_id = $request->truck_id;
        ...
        $truck_post->status_phone = $request->status_phone == "1" ? Auth::user()->phone : null;
        $truck_post->status_email = $request->status_email == "1" ? Auth::user()->email : null;
    ...
    public function QuickRateLookup(Request $request)
    {
        ...
        if ($request->eq_type_id == "4") {
            $eq_type_id = $request->eq_type_id;
            $fuel_surcharge = "Rates for Reefer iclude a fuel surcharge of $0.42/mi";
        } else if ($request->eq_type_id == "2") {
            $eq_type_id = $request->eq_type_id;
            $fuel_surcharge = "Rates for Flatbeds iclude a fuel surcharge of $0.46/mi";
        } else {
            $eq_type_id = 1;
            $fuel_surcharge = "Rates for Vans iclude a fuel surcharge of $0.38/mi";
