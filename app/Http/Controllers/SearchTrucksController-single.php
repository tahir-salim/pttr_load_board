    ...
    public function search_trucks(Request $request)
    {
        ...
        elseif(isset($request->origin) && count($request->origin) > 0 && isset($request->destination) && count($request->destination) > 0){
            ...
            elseif(strpos($request->origin[0], 'state_') === 0 && strpos($request->destination[0], 'city_') === 0){
                ...
                $des_lng = $dest_city->longitude;
                    ...
                    $exact_trucks =  TruckPost::select(['id'])->whereIn('id', $exact_trucks)->whereIn('origin_state_id', $origin_state_ids)->where('is_posted', 1)->get()->pluck('id')->toArray();
                    $included_trucks =  TruckPost::select(['id'])->whereIn('id', $included_trucks)->whereIn('origin_state_id', $origin_state_ids)->where('is_posted', 1)->get()->pluck('id')->toArray();
        ...
        if ($request->equipment_details != null) {
            ...
            if ($request->length != null) {
                ...
                $trucks->whereHas('trucks', function($q) use ($length) {
            ...
            if ($request->weight != null) {
                ...
                $trucks->whereHas('trucks', function($q) use ($weight) {
        ...
        if ($request->has('searchback') && $request->searchback != null ) {
            ...
            $trucks->WhereBetween('created_at' , [$previousHours, Carbon::now()]);
        }

        $trucks = $trucks->orderBy('id','Desc')->paginate(25);
        ...
        $trucks->transform(function ($truckpost) use ($org_lat ,  $org_lng , $des_lng , $des_lat) {
            ...
            $truckpost['company_mc'] =  $company ? $company->mc : null;
            $truckpost['company_dot'] =  $company ? $company->dot : null;
            $truckpost['status_phone'] = $truckpost->status_phone == 1 ? $truckpost->user ? $truckpost->user->phone : null : null;
            $truckpost['status_email'] = $truckpost->status_email == 1 ? $truckpost->user ? $truckpost->user->email : null : null;
            ...
            $truckpost['reference_id'] = $truckpost->trucks ?   $truckpost->trucks->reference_id != null ? $truckpost->trucks->reference_id : '-' : '-';
            $truckpost['comment'] =  $truckpost->comment != null ? $truckpost->comment : '-' ;
